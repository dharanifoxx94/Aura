...
with no ~/.eidolon_vault/config.yaml to bleed in.
"""
import copy
import logging
import os
...
class TestLoadConfigLayers:
    ...
    every test a fake home with no .eidolon_vault/config.yaml present."""
    ...
    def test_empty_home_yields_defaults(self):
        """With no ~/.eidolon_vault/config.yaml (empty fake home), only defaults load."""
        cfg = load_config()
        assert cfg["simulation"]["max_agents"] == DEFAULT_CONFIG["simulation"]["max_agents"]
        assert cfg["llm"]["request_timeout"]   == DEFAULT_CONFIG["llm"]["request_timeout"]
    ...
    def test_both_layers_loaded_in_order(self, tmp_path):
        ...
        (eidolon_vault_dir / "config.yaml").write_text("simulation:\n  max_agents: 5\n")
        (tmp_path / "config.yaml").write_text("simulation:\n  max_turns: 7\n")
        cfg = load_config()
...
class TestEnvOverrides:
    ...
    def test_no_eidolon_vault_vars_yields_defaults(self):
        cfg = load_config()
        assert cfg["simulation"]["max_agents"] == DEFAULT_CONFIG["simulation"]["max_agents"]
        assert cfg["llm"]["request_timeout"]   == DEFAULT_CONFIG["llm"]["request_timeout"]
