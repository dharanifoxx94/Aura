#!/usr/bin/env bash
# =============================================================================
# Eidolon Vault Knowledge Pipeline — setup & smoke-test
#
# Fixes:
#   1. Writes agent_personas.yaml into eidolon-vault/
#   2. Patches missing _escape_like() into memory_store.py
#   3. Fixes ~/.eidolon-vault/config.yaml: un-indented keys under input:
#   4. eidolon-vault learn --url <TEST_URL> --turns 3
#   5. eidolon-vault status
#
# Usage:
#   bash setup_and_test.sh
#   TEST_URL=https://other-url bash setup_and_test.sh
#   EIDOLON_VAULT_DIR=/custom/path bash setup_and_test.sh
# =============================================================================

set -euo pipefail

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'
CYAN='\033[0;36m'; BOLD='\033[1m'; RESET='\033[0m'
info()   { echo -e "${CYAN}[INFO]${RESET}  $*"; }
ok()     { echo -e "${GREEN}[OK]${RESET}    $*"; }
warn()   { echo -e "${YELLOW}[WARN]${RESET}  $*"; }
die()    { echo -e "${RED}[ERROR]${RESET} $*" >&2; exit 1; }
banner() { echo -e "\n${BOLD}$*${RESET}"; }

TEST_URL="${TEST_URL:-https://www.hackster.io/adam-taylor/understanding-rfsoc-bbf149}"
EIDOLON_VAULT_CONFIG="${HOME}/.eidolon-vault/config.yaml"

# ── Step 0: Locate Eidolon Vault install ───────────────────────────────────────────────
banner "=== Step 0: Locating Eidolon Vault install ==="

if [[ -z "${EIDOLON_VAULT_DIR:-}" ]]; then
    EIDOLON_VAULT_BIN="$(command -v eidolon-vault 2>/dev/null || true)"
    [[ -n "$EIDOLON_VAULT_BIN" ]] || die "'eidolon-vault' not found on PATH. Activate your venv first."
    REAL_BIN="$(readlink -f "$EIDOLON_VAULT_BIN")"
    info "eidolon-vault binary: $REAL_BIN"
    EIDOLON_VAULT_DIR=""
    SEARCH="$(dirname "$REAL_BIN")"
    for _ in 1 2 3 4 5; do
        SEARCH="$(dirname "$SEARCH")"
        if [[ -f "${SEARCH}/eidolon-vault/__init__.py" ]]; then EIDOLON_VAULT_DIR="$SEARCH"; break; fi
    done
    if [[ -z "$EIDOLON_VAULT_DIR" ]]; then
        for candidate in "${HOME}/EIDOLON_VAULT_v-1.4" "${HOME}/Eidolon Vault" "$(pwd)"; do
            if [[ -f "${candidate}/eidolon-vault/__init__.py" ]]; then EIDOLON_VAULT_DIR="$candidate"; break; fi
        done
    fi
    [[ -n "$EIDOLON_VAULT_DIR" ]] || die "Cannot find Eidolon Vault project dir. Set EIDOLON_VAULT_DIR=/your/path"
fi

EIDOLON_VAULT_PKG="${EIDOLON_VAULT_DIR}/eidolon-vault"
[[ -f "${EIDOLON_VAULT_PKG}/__init__.py" ]] || die "eidolon-vault package not found at ${EIDOLON_VAULT_PKG}"
ok "Eidolon Vault project root: ${EIDOLON_VAULT_DIR}"

# ── Step 1: Write agent_personas.yaml ─────────────────────────────────────────
banner "=== Step 1: Writing agent_personas.yaml ==="
echo '' | base64 -d > "${EIDOLON_VAULT_PKG}/agent_personas.yaml"
ok "Written: ${EIDOLON_VAULT_PKG}/agent_personas.yaml"

# ── Step 2: Patch _escape_like into memory_store.py ───────────────────────────
banner "=== Step 2: Patching memory_store.py (_escape_like) ==="
MEMORY_STORE="${EIDOLON_VAULT_PKG}/memory_store.py"
if grep -q "^def _escape_like" "$MEMORY_STORE"; then
    ok "_escape_like already defined — skipping."
else
    cp "$MEMORY_STORE" "${MEMORY_STORE}.bak"
    info "Backup: ${MEMORY_STORE}.bak"
    echo 'CgpkZWYgX2VzY2FwZV9saWtlKHM6IHN0cikgLT4gc3RyOgogICAgIiIiRXNjYXBlIFNRTGl0ZSBMSUtFIHdpbGRjYXJkcyDigJQgd2FzIG1pc3NpbmcgZnJvbSBtZW1vcnlfc3RvcmUucHkuIiIiCiAgICByZXR1cm4gcy5yZXBsYWNlKCJcXCIsICJcXFxcIikucmVwbGFjZSgiJSIsICJcXCUiKS5yZXBsYWNlKCJfIiwgIlxcXyIpCg==' | base64 -d >> "$MEMORY_STORE"
    echo '' | base64 -d | python3 - "$MEMORY_STORE"
    ok "memory_store.py patched and verified."
fi

# ── Step 3: Fix ~/.eidolon-vault/config.yaml ───────────────────────────────────────────
banner "=== Step 3: Fixing ~/.eidolon-vault/config.yaml ==="
if [[ ! -f "$EIDOLON_VAULT_CONFIG" ]]; then
    warn "No config found — running eidolon-vault init."
    eidolon-vault init
else
    info "Lines 35-45 (before fix):"
    sed -n '35,45p' "$EIDOLON_VAULT_CONFIG" | cat -A
    cp "$EIDOLON_VAULT_CONFIG" "${EIDOLON_VAULT_CONFIG}.bak"
    info "Backup: ${EIDOLON_VAULT_CONFIG}.bak"
    echo '' | base64 -d | python3 - "$EIDOLON_VAULT_CONFIG"
    YAML_STATUS="$(echo 'aW1wb3J0IHlhbWwsIHN5cwp0cnk6CiAgICB5YW1sLnNhZmVfbG9hZChvcGVuKHN5cy5hcmd2WzFdKSkKICAgIHByaW50KCJPSyIpCmV4Y2VwdCB5YW1sLllBTUxFcnJvciBhcyBlOgogICAgcHJpbnQoc3RyKGUpKQo=' | base64 -d | python3 - "$EIDOLON_VAULT_CONFIG")"
    if [[ "$YAML_STATUS" == "OK" ]]; then
        ok "config.yaml parses cleanly."
        info "Lines 35-45 after fix:"
        sed -n '35,45p' "$EIDOLON_VAULT_CONFIG"
    else
        warn "Still invalid: $YAML_STATUS"
        info "Lines 35-45 with hidden chars:"
        sed -n '35,45p' "$EIDOLON_VAULT_CONFIG" | cat -A
        warn "Restoring backup — eidolon-vault will fall back to defaults."
        cp "${EIDOLON_VAULT_CONFIG}.bak" "$EIDOLON_VAULT_CONFIG"
    fi
fi

# ── Step 4: Verify imports ────────────────────────────────────────────────────
banner "=== Step 4: Verifying Python imports ==="
echo '' | base64 -d | python3 - "${EIDOLON_VAULT_DIR}"
ok "All pipeline modules import OK."

# ── Step 5: eidolon-vault learn ────────────────────────────────────────────────────────
banner "=== Step 5: eidolon-vault learn --url '${TEST_URL}' --turns 3 ==="
info "Making real LLM calls — expect ~1-3 minutes."
echo ""
set +e
eidolon-vault learn --url "$TEST_URL" --turns 3
LEARN_EXIT=$?
set -e
echo ""
if [[ $LEARN_EXIT -eq 0 ]]; then
    ok "eidolon-vault learn succeeded."
else
    warn "eidolon-vault learn exited with code ${LEARN_EXIT} — check output above."
fi

# ── Step 6: eidolon-vault status ───────────────────────────────────────────────────────
banner "=== Step 6: eidolon-vault status ==="
eidolon-vault status || true
echo ""
ok "All done."
