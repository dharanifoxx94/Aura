#!/usr/bin/env bash
# =============================================================================
# Eidolon Vault — mitigate timeout / unpack bugs and smoke-test
#
# Fixes:
...
#   6. Smoke-tests: eidolon-vault learn --url <TEST_URL> --turns 3
#
# Usage:
#   bash fix_and_test.sh
#   TEST_URL=https://other-url bash fix_and_test.sh
#   EIDOLON_VAULT_DIR=/custom/path bash fix_and_test.sh
# =============================================================================
...
if [[ -z "${EIDOLON_VAULT_DIR:-}" ]]; then
    ...
    [[ -n "$EIDOLON_VAULT_BIN" ]] || die "'eidolon-vault' not found on PATH. Activate venv first."
...
[[ -f "$EIDOLON_VAULT_CONFIG" ]] || { warn "No config — running eidolon-vault init."; eidolon-vault init; }
cp "$EIDOLON_VAULT_CONFIG" "${EIDOLON_VAULT_CONFIG}.bak2"
info "Backup: ${EIDOLON_VAULT_CONFIG}.bak2"
...
# ── Smoke test ────────────────────────────────────────────────────────────────
banner "=== Smoke test: eidolon-vault learn --url '${TEST_URL}' --turns 3 ==="
...
if [[ $LEARN_EXIT -eq 0 ]]; then
    ...
    warn "eidolon-vault learn exited ${LEARN_EXIT}."
fi
